...
covid_df$date <- as.Date(covid_df$date, format = "%m/%d/%y")
...
ggplot(data=florida_df, aes(x=cases, y=deaths, group=1)) + geom_line()

## Add lines for New York and California to the plot
ggplot(data=florida_df, aes(x=cases, group=1)) +
